...
const Chatbox = () => {
  ...
  const performSentimentAnalysis = async (text) => {
    try {
      const response = await axios.post('http://127.0.0.1:8000/predict_sentiment', { text });
  ...
  const handleInputSubmit = async (e) => {
    ...
    if (inputValue) {
      ...
      setOverallSentiment((prevSentiment) =>
        prevSentiment + sentimentModifier
      );
  ...
  return (
    ...
    <div className="max-w-screen-xl px-4 py-8 mx-auto space-y-12 lg:space-y-20 lg:py-24 lg:px-6 bg-gray-700">
        ...
        <h2 className="mb-4 text-5xl font-extrabold tracking-tight text-gray-900 dark:text-white">Chatbot</h2>
        </div>
          ...
          <div className="flex flex-col space-y-4">
            {messages.map((message, index) => (
              ...
              className={`max-w-md ${message.color} text-white
                          ${message.sender === 'user' ? 'self-start' : 'self-end'} rounded-lg p-2`}
              >
              <p className="max-w-1xl 
                font-medium
              text-slate-50 md:text-normal lg:text-normal dark:text-slate-50">{message.content}</p>
              </div>
          ...
          <div className="flex items-center px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-700">
            ...
            <label className="inline-flex relative items-center mr-5 cursor-pointer">
                ...
                <span className="ml-2 text-sm font-medium text-gray-900">
                    <img className="hidden w-6 mb-0 ml-3 rounded-lg lg:mb-0 lg:flex" src = {agent_png} />
                </span>
            </label>
                        
            {/* chat text */}
            <textarea id="chat" rows="1" disabled={chatEnded} 
            ...
            {/* submit button */}
            <button disabled={chatEnded} type="submit" className="inline-flex justify-center p-2 text-blue-600 rounded-full cursor-pointer hover:bg-blue-100 dark:text-blue-500 dark:hover:bg-gray-600">
            ...
            <button onClick={handleEndChat} disabled={chatEnded} type="submit" className="inline-flex justify-center p-2 text-blue-600 rounded-full cursor-pointer hover:bg-blue-100 dark:text-blue-500 dark:hover:bg-gray-600">
      ...
      {chatEnded && <>
        <br />
        <hr /><br />
        <h2 className="mb-4 text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">Agent Perfomance Stats:</h2>
        <br />
        {/* {(overallSentimentString(overallSentiment))} */}
        
        <div>

          {console.log(handleMessageColor((overallSentimentString(overallSentiment))))}

                    <h3 className="mb-2 text-2xl font-bold dark:text-white">Overall Customer Sentiment</h3>
                    <p className={`text-white
                             ...
                             dark: ${handleMessageColor((overallSentimentString(overallSentiment)))} dark:hover:bg-opacity-80 focus:outline-none dark:focus:ring-purple-800
                             capitalize`}>
                    {(overallSentimentString(overallSentiment))}</p>
        </div>
        <br />
        <div>
                    <h3 className="mb-2 text-2xl font-bold dark:text-white">Experience Points</h3>
